            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 rounded-2xl border-4 border-black/10 dark:border-white/10 overflow-hidden shadow-xl will-change-transform select-none",
          ...
          <div className="flex items-center gap-2">
            {items.map((it, idx) => (
              <button
                ...
                className={cn("h-2 w-2 rounded-full transition", idx === active ? "bg-white" : "bg-white/30 hover:bg-white/50")}
                aria-label={`Go to ${it.title}`}
              />
            ))}
          </div>
          {activeItem.href ? (
            <Link href={activeItem.href} target="_blank" rel="noreferrer" className="text-white/60 hover:text-white transition">
    ...
    <div className="relative h-full w-full">
      <div className="absolute inset-0">
        {item.imageSrc ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-black/50 text-sm text-neutral-400">Görsel Yok</div>
        )}
      </div>
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#020617]/90 via-black/40 to-transparent" />
      <div className="relative z-10 flex h-full flex-col justify-end p-6">
        <div className="truncate text-xl font-black text-white uppercase tracking-wider">{item.title}</div>
        {item.description ? <div className="mt-2 line-clamp-2 text-sm text-neutral-300 font-light">{item.description}</div> : null}
      </div>
    </div>
  );
}
