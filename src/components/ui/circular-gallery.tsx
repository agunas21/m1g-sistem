...
const CircularGallery = React.forwardRef<HTMLDivElement, CircularGalleryProps>(
  ({ items, className, radius = 600, autoRotateSpeed = 0.02, ...props }, ref) => {
    ...
    // Safely handle empty items
    if (!items || items.length === 0) {
      return <div className="text-center p-8 text-neutral-500">Galeri için henüz resim eklenmemiş.</div>;
    }
              ...
              >
                <div className="relative w-full h-full rounded-lg shadow-2xl overflow-hidden group border border-white/10 bg-black/70 backdrop-blur-lg">
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-4 bg-gradient-to-t from-black/90 via-black/50 to-transparent text-white">
                    ...
                    {item.binomial && <em className="text-sm italic text-red-400">{item.binomial}</em>}
                    {item.photo.by && <p className="text-xs mt-2 opacity-70">Ekleyen: {item.photo.by}</p>}
                  </div>
                </div>
              </div>
            );
