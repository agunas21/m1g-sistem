...
export default function DepoError({
  error,
  reset,
}: {
  ...
  return (
    <div className="min-h-[75vh] flex flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className="text-xl font-black text-white uppercase tracking-widest mb-2">Depo Paneli Yüklenemedi</h2>
      <p className="text-neutral-400 text-xs font-mono max-w-md mb-6 bg-black/50 p-3 rounded-xl border border-white/10 break-all">
        {error?.message || "Envanter verisi veya arayüz yüklenirken bir istemci hatası oluştu."}
      </p>
      <button
        onClick={() => reset()}
      ...
      >
        <RefreshCw size={16} /> Depo Panelini Yeniden Yükle
      </button>
    </div>
  );
}
